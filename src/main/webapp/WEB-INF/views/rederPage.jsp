...
<body>
	...
	<div class="background-color-Top">
		...
		<!-- LoginPageControllerクラスのformクラスの処理が、実行される。 -->
			...
			<div>
				...
				<div class="px-4">
					<form:form action="/R_GroupSearch" modelAttribute="result" method="post">
						<div class="p-1">
							<form:hidden path="userId" value="${fn:escapeXml(users.userId)}"/>
							<form:input class="input" path="groupName"/>		<!--/loginConfirmのパラメータ(groupSelect)に紐づいている  -->
					...
					<c:choose>						
						...
						<c:when test="${typeMatch eq 'ok'}">	
							<form:form action="/rederGroupPage" modelAttribute="result" method="post">
				...
				<c:forEach items="${GroupList}" var="Group">
					...
					<tr>
						...
						<form:form action="/rederGroupPage" modelAttribute="result" method="post">
							...
							<td> 
							<div class="d-flex p-1 bd-highlight">
								<div class="px-3"><b>${fn:escapeXml(Group.groupName)}</b></div>
								
								<!-- groupIdは、(userGroupList)から、取得した,groupIdをフォームにして一致したグループIDだけをgroupPageに渡す -->
							<form:hidden path="groupId" value="${fn:escapeXml(Group.groupId)}"/>
	...
	<div class="mx-3">					<!-- コントロラークラスに推移先を指定したら変更する -->
